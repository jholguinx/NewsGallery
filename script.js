...
function addnews() {
      var cardscount = document.querySelector(".cards");
      if (cardscount.innerText === '') {
            for (let index = 0; index < noticias.length; index++) {
                  let titulo = noticias[index].titulo;
                  let imgURL = noticias[index].imgUrl;
                  let desc = noticias[index].descripcion;
                  let fecha = noticias[index].fecha;
                   var news = document.createElement("p");
                   news.classList.add("card");
                   var imagen = document.createElement("img");
                   imagen.src = imgURL;
                   imagen.classList.add("image");
                   news.appendChild(imagen);
                   var title = document.createElement("p");
                   title.innerText = titulo;
                   news.appendChild(title);
                   var title2 = document.createElement("p");
                   title2.innerText = desc;
                   news.appendChild(title2);
                   var fechad = document.createElement("p");
                   fechad.innerText = fecha;
                   news.appendChild(fechad);
                   document.querySelector(".cards").appendChild(news);
                  
            }
      }  else {alert("No hay noticias para agregar");}
